...
_GEOMETRY_PATH_ENV = "MATHVISION_GEOMETRY_PATH"
_EPSILON = 1e-9
_ROUND_DIGITS = 6
...
def patch_scene_for_geometry_export(scene_cls):
    ...
    if not geometry_path or getattr(scene_cls, "_mathvision_geometry_export_patched", False):
        return scene_cls

    scene_source_path = _resolve_scene_source_path(scene_cls)
    helper_source_path = Path(__file__).resolve()

    class MathVisionGeometryExportScene(scene_cls):
        def setup(self):
            self._mathvision_geometry_path = geometry_path
            self._mathvision_geometry_export_error = None
            self._mathvision_scene_source_path = scene_source_path
            self._mathvision_helper_source_path = helper_source_path
            self._mathvision_play_index = 0
            self._mathvision_sample_index = 0
            self._mathvision_observed_render_frame_count = 0
            self._mathvision_samples = []
            self._mathvision_play_records = []
        ...
        def add(self, *mobjects):
            source_hint = self._mathvision_resolve_source_hint()
            semantic_hints = _extract_positional_semantic_hints(
                source_hint=source_hint,
                scene_source_path=self._mathvision_scene_source_path,
                method_names={"add"},
                arg_count=len(mobjects),
            )
            self._mathvision_register_roots(mobjects, source_hint, semantic_hints=semantic_hints)
        ...
        def add_fixed_in_frame_mobjects(self, *mobjects):
            ...
            parent_method = getattr(
                super(MathVisionGeometryExportScene, self),
        ...
        def play(
            ...
            **kwargs,
            ...
            self._mathvision_play_records.append(play_record)
            self._mathvision_current_play = play_record

            self._mathvision_register_targets_from_play_args(
                args,
                source_hint,
                play_argument_hints=play_argument_hints,
            )
            play_record["expected_removed_object_ids"] = self._mathvision_expected_removed_object_ids(
            ...
            try:
                start_collected = self._mathvision_collect_visible_objects(
                    source_hint=source_hint,
                    play_index=play_record["play_index"],
                    scene_time=getattr(self, "time", 0.0),
                )
                ...
                if (
                    ...
                    pre_removal_collected = self._mathvision_collect_visible_objects(
                        source_hint=source_hint,
                        play_index=play_record["play_index"],
                        scene_time=getattr(self, "time", 0.0),
                        extra_descriptors=pre_removal_descriptors,
                    )
                    self._mathvision_capture_sample(
                ...
                current_play = self._mathvision_current_play
                if current_play is not None:
                    ...
                    final_collected = self._mathvision_collect_visible_objects(
                        source_hint=source_hint,
                    ...
                    if final_visibility_event is not None:
                        sample = self._mathvision_capture_sample(
                        ...
                        self._mathvision_note_visibility_change(
                            current_play,
                            sample,
        ...
        def _mathvision_install_geometry_hook(self):
            if getattr(self, "_mathvision_geometry_hook_installed", False):
                return
            ...
            def wrapped_render(renderer_self, scene, time, moving_mobjects=None):
                if scene is self:
                    try:
                        self._mathvision_observed_render_frame_count += 1
                        self._mathvision_handle_render_frame(
                            renderer_self=renderer_self,
                            animation_time=time,
                        )
                    except Exception as exc:
                        self._mathvision_note_geometry_error(exc)
                return original_render(scene, time, moving_mobjects)

            self.renderer.render = types.MethodType(wrapped_render, self.renderer)
            self._mathvision_geometry_hook_installed = True

        def _mathvision_handle_render_frame(self, renderer_self, animation_time):
            current_play = self._mathvision_current_play
            ...
            collected = self._mathvision_collect_visible_objects(
                source_hint=capture_source_hint,
                play_index=current_play["play_index"],
                scene_time=scene_time,
            )
            visibility_event = self._mathvision_detect_visibility_change(
                current_play=current_play,
            ...
            if visibility_event is not None:
                ...
                self._mathvision_note_visibility_change(current_play, sample, visibility_event)

        def _mathvision_note_geometry_error(self, exc):
            if self._mathvision_geometry_export_error is None:
                self._mathvision_geometry_export_error = f"{type(exc).__name__}: {exc}"

        def _mathvision_resolve_source_hint(self):
            return _resolve_source_hint(
                scene_source_path=self._mathvision_scene_source_path,
                helper_source_path=self._mathvision_helper_source_path,
            )

        def _mathvision_register_roots(self, mobjects, source_hint, semantic_hints=None):
            ...
            for index, mobject in enumerate(mobjects):
                ...
                self._mathvision_register_mobject(
                    mobject,
                    source_hint=source_hint,
        ...
        def _mathvision_expected_removed_object_ids(
            ...
            for descriptor in descriptors or []:
                ...
                if not _should_track_object(mobject):
                    continue
                registry_entry = self._mathvision_register_mobject(
                    mobject,
                    source_hint=source_hint,
                    play_index=play_index,
        ...
        def _mathvision_register_mobject(
            self,
            mobject,
            source_hint,
            play_index,
            ...
            if stable_id is None:
                stable_id = f"mob-{self._mathvision_next_object_number:04d}"
                self._mathvision_next_object_number += 1
                self._mathvision_object_ids[object_id] = stable_id
                self._mathvision_object_registry[stable_id] = {
                    "stable_id": stable_id,
        ...
        def _mathvision_collect_visible_objects(
            ...
            for mobject in roots:
                ...
                if object_id in seen_object_ids:
                    continue
                root_entry = self._mathvision_register_mobject(
                    mobject,
                    source_hint=source_hint,
                    play_index=play_index,
                    scene_time=_safe_float(scene_time),
                )
                added = self._mathvision_collect_layout_elements(
                    mobject=mobject,
            ...
            for descriptor in extra_descriptors or []:
                ...
                if object_id in seen_object_ids:
                    continue
                extra_entry = self._mathvision_register_mobject(
                    mobject,
                    source_hint=source_hint,
                    play_index=play_index,
                    scene_time=_safe_float(scene_time),
        ...
        def _mathvision_collect_layout_elements(
            self,
            mobject,
            registry_entry,
            source_hint,
            ...
            if _should_descend_layout_children(mobject, child_descriptors):
                ...
                for child_index, child in child_descriptors:
                    child_entry = self._mathvision_register_mobject(
                        child,
                        source_hint=source_hint,
                        play_index=play_index,
                        scene_time=_safe_float(scene_time),
                    ...
                    self._mathvision_collect_layout_elements(
                        mobject=child,
        ...
        def _mathvision_detect_visibility_change(self, current_play, visible_signature):
            ...
            return {
                ...
                "added_objects": _object_refs_from_ids(self._mathvision_object_registry, added_ids),
        ...
        def _mathvision_note_visibility_change(self, current_play, sample, visibility_event):
        ...
        def _mathvision_capture_sample(
            self,
            sample_role,
            trigger,
            animation_time,
            ...
            self._mathvision_sample_index += 1
            sample_id = f"sample-{self._mathvision_sample_index:04d}"
            sample = {
                "sample_id": sample_id,
                "play_index": play_index,
            ...
            self._mathvision_samples.append(sample)
        ...
        def _mathvision_write_geometry_file(self):
            payload = {
                ...
                "sample_count": len(self._mathvision_samples),
                "samples": self._mathvision_samples,
            }
            if self._mathvision_geometry_export_error is not None:
                payload["export_error"] = self._mathvision_geometry_export_error

            output_path = Path(self._mathvision_geometry_path)
        ...
        def render(self, preview=False):
            try:
                ...
                try:
                    ...
                    final_source_hint = (
                        current_play_source_hint(self._mathvision_play_records[-1])
                        if self._mathvision_play_records
                        else self._mathvision_resolve_source_hint()
                    )
                    final_collected = self._mathvision_collect_visible_objects(
                    ...
                    self._mathvision_write_geometry_file()
                except Exception as exc:
                    self._mathvision_note_geometry_error(exc)
                    try:
                        self._mathvision_write_geometry_file()
    ...
    MathVisionGeometryExportScene.__name__ = scene_cls.__name__
    MathVisionGeometryExportScene.__qualname__ = scene_cls.__qualname__
    MathVisionGeometryExportScene.__module__ = scene_cls.__module__
    MathVisionGeometryExportScene.__doc__ = scene_cls.__doc__
    MathVisionGeometryExportScene._mathvision_geometry_export_patched = True
    return MathVisionGeometryExportScene
