...
public final class CodeEvaluationPrompts {
    ...
    private static final String REVIEW_SYSTEM_MANIM =
            "You are a senior Manim code reviewer.\n"
                    ...
                    + "- `manim_coordinate_mapping` [MANDATORY]: when storyboard `coordinate_bounds` exists, storyboard world-coordinate geometry must be mapped through `axes.c2p(...)` or a clearly named helper wrapping `c2p`; raw Manim frame coordinates are allowed only for fixed overlays, titles, camera/UI placement, and other non-storyboard-geometry elements. Fail this check when points, lines, curves, polygons, markers, or geometry-attached labels use raw coordinates such as `Dot([x, y, 0])`, `Line([x1, y1, 0], [x2, y2, 0])`, or `.move_to([x, y, 0])` instead of the coordinate-system mapping.\n"
