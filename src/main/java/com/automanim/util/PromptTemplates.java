...
public final class PromptTemplates {
    ...
    public static final String FOUNDATION_CHECK_SYSTEM =
            ...
            + "3. 只要该概念通常需要高中、竞赛、大学层面的知识，就不是基础概念。\n"
            + "4. 如果一个概念虽然常见，但已经偏离最终主题，或者需要公式推导、抽象定义或更高层先修知识，也不是基础概念。\n"
            + "5. 宁可判断为“否”，也不要过早停止拆解或向无关方向扩散。\n"
            + "\n"
            + "通常可视为基础概念的例子（回答“是”）：\n"
            + "- 数、加减乘除\n"
            + "- 分数、小数、百分数\n"
    ...
    public static final String PREREQUISITES_SYSTEM =
            ...
            + "4. 优先选择更基础、更直观、更接近初中认知水平，同时又最贴近当前主题链路的概念。\n"
            + "5. 如果某个候选前置概念本身超过初中理解范围，就继续向下替换成更基础、但仍与主题紧密相关的概念。\n"
            + "6. 返回结果应尽量使每个概念都能被普通初中生理解，而不是停留在高中或大学概念上。\n"
            + "7. 不要列出过于底层的初中内容，如四则运算、分数、小数、基础几何、简单方程，除非它们对当前概念确实不可缺少。\n"
            + "8. 不要给历史背景、应用背景、宽泛类别词或离题概念；尽量具体、可教、可解释。\n"
            + "9. 最多返回 3 到 5 个前置概念，按重要性从高到低排序。\n"
            + "\n"
            + "正确倾向示例：\n"
